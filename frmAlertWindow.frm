...
Begin VB.Form frmAlertWindow 
   ...
   Begin VB.Image Image1 
      Height          =   240
      Left            =   2040
      Picture         =   "frmAlertWindow.frx":000C
      Top             =   0
      Width           =   240
   End
...
Private Sub Form_Load()
  ...
  Dim OSV        As OSVersionInfo
    ...
    Me.Move scrnRight - 2400, m_iScrnBottom, 2400, 2000
...
Private Sub Form_Click()
    ...
    '/* Close by user
    If Not m_bCodeClose Then m_iCounter = 0
...
Public Sub DisplayMessage(ByVal SMessage As String, _
                            ...
                            Optional Sound As SoundType = 20)
    ...
    If DisplaySeconds < 1& Then '/* Manual Close
        ...
        If DisplaySeconds = 0& Then
            ...
            SMessage = "Clic para cerrar" & vbNewLine & SMessage
    ...
    Else '/* Auto Close
        ...
        SMessage = vbNewLine & SMessage
        m_iCounter = DisplaySeconds * 100&
    End If
    

    '/* Resize the Form's height based on the amount of text to display
    '/* If more then one is showing then fix the height to standard to insure no overlap
    Me.ScaleMode = vbPixels
    If m_iCounter = 1 Then
        lblMessage.Move 5, 5, Me.ScaleWidth - 10
    Else
        lblMessage.Move 5, 10, Me.ScaleWidth - 10
    End If
    lblMessage.Caption = SMessage
    If m_iWindowCount = 1 And bAutoFit Then
        Me.Height = (lblMessage.top + lblMessage.Height + 15) * Screen.TwipsPerPixelY
    End If
    '/* Fix the maximum height to be not greater then standard
    ...
    If m_bFade Then
        ...
        m_iChangeSpeed = 100&
        Me.top = m_iScrnBottom - (Me.Height * m_iWindowCount)
        '/* prevent it from going over the top of the screen
        If Me.top < 0 Then Me.top = 0
        Call MakeTransparent(m_iChangeSpeed)
        Call SetOnTop(True)
    Else
        '/* Move distance per millisecond
        If m_iWindowCount > 1 Then
            m_iChangeSpeed = 100&
        Else
            m_iChangeSpeed = 50&
        End If
    ...
    If bSquare Then
        ...
        Me.Line (Me.ScaleWidth - 1, Me.ScaleHeight - 1)-(Me.ScaleWidth - 1, 0), vbButtonFace 'vbButtonShadow
        Me.Line (Me.ScaleWidth - 1, Me.ScaleHeight - 1)-(0, Me.ScaleHeight - 1), vbButtonFace 'vbButtonShadow
        Me.Line (Me.ScaleWidth - 1, 0)-(0, 0), vbButtonFace
        Me.Line (0, Me.ScaleHeight - 1)-(0, 0), vbButtonFace
        
        Me.Line (Me.ScaleWidth - 2, Me.ScaleHeight - 2)-(Me.ScaleWidth - 2, 1), iBackColor 'vbButtonShadow
        Me.Line (Me.ScaleWidth - 2, Me.ScaleHeight - 2)-(1, Me.ScaleHeight - 2), iBackColor 'vbButtonShadow
...
Private Sub Timer1_Timer()
    ...
    If m_bFade Then '/* Fade
        ...
        If m_iCounter > 0 Then '/* Fade In then Wait
            If m_iChangeSpeed > 0 Then
                m_iChangeSpeed = m_iChangeSpeed - 2
                Call MakeTransparent(m_iChangeSpeed)
            Else
                '/* Wait
                If Not m_bManualClose Then m_iCounter = m_iCounter - 1
            End If
            
        Else '/* Fade Out then Close
            If m_iChangeSpeed <= 100 Then
                '/* Fade out
                 m_iChangeSpeed = m_iChangeSpeed + 2
                 Call MakeTransparent(m_iChangeSpeed)
            Else
    ...
    Else '/* Move
        ...
        If m_iCounter > 0 Then '/* Move Up then Wait
            If Me.top > 0 And Me.top > m_iScrnBottom - (Me.Height * m_iWindowCount) Then
                '/* Move Up
                Me.top = Me.top - m_iChangeSpeed
            Else
                '/* Wait
...
Private Sub GetDisplayPosition(ByVal SetPosition As Boolean)
    ...
    '/* This option can be removed by always setting m_iWindowCount = 1 and commenting out the rest of this sub
...
Public Property Let CloseActivate(ByVal vNewValue As Boolean)
    ...
    If vNewValue Then m_iCounter = 0
End Property
